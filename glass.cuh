...
// The measured body-dispatch table consumed by the bare face's wrappers below
// (op/body enums + dispatch_body(); regenerated by bench/tune.py --legs body).
#include "./glass-dispatch.cuh"
...
/*  The bare glass:: face. Names with NO moved dispatch cell resolve to the
    SAME entities as glass::block:: via this directive (function-pointer
    identical — pinned by static_asserts in test_defaults). Ops with measured
    non-block cells are shadowed by the wrappers in src/base/dispatch.cuh
    below (declarations directly in a namespace beat using-directive names on
    qualified lookup), which route each (op, N, dtype) cell to the body the
    dispatch_body() table picked — block twin verbatim, or the warp/thread
    twin under the same block-scope calling contract. The warp/thread
    sub-namespaces are hoisted back to glass:: scope: they are written inline
    in the base headers, so under the block wrap they land at block::warp /
    block::thread; the aliases keep the public spellings. */
using namespace block;
namespace warp   = block::warp;
namespace thread = block::thread;

/*  Measured-default wrappers for the bare face (shadow ops with moved cells). */
#include "./src/base/dispatch.cuh"
}  // namespace glass
