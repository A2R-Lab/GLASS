...
// independent GEMMs across one threadIdx.x range. Callers (notably RBD-SOLVER)
...
//
// Common pattern in RBD-SOLVER's EE-pose-gradient: a single 4×4 transform A is
 ...
 * in RBD-SOLVER's EE-pose-gradient (one transform applied to many destinations).
