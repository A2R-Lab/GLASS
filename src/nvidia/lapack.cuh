...
//
// Currently supported (matching RBD-SOLVER's potrf + trsm contract):
//   potrf<T, N>(A, smem)
 ...
 * @param  smem          Shared scratch (>= potrf_scratch_bytes<...>()).
 */
...
/**
 * @brief Shared-memory bytes needed by `potrf<...>` (host-callable, constexpr).
 * @tparam T T scalar; @tparam N dim; @tparam BLOCK_THREADS pinned BlockDim; @tparam SM_VAL SM arch.
 */
template <typename T, uint32_t N, uint32_t BLOCK_THREADS = 0, uint32_t SM_VAL = SMS>
constexpr std::size_t potrf_scratch_bytes() { return 0; }

/**
 * @brief Thread count cuSOLVERDx wants for `potrf<...>` (host-callable, constexpr).
 * @tparam T scalar; @tparam N dim; @tparam BLOCK_THREADS pinned BlockDim; @tparam SM_VAL SM arch.
 */
template <typename T, uint32_t N, uint32_t BLOCK_THREADS = 0, uint32_t SM_VAL = SMS>
constexpr uint32_t potrf_threads() { return 256; }
...
#define _GLASS_CHOL_NO_BD(N, CT, ARCH)                                                            \
    ...
    template <>                                                                               \
    __device__ inline void potrf<CT, N, 0, ARCH, true>(CT* A, char* smem)        \
    {                                                                                         \
        _nvidia_chol_impl_##N##_##CT##_bd0_sm##ARCH::template run<true>(A, smem);                    \
    }                                                                                         \
    template <>                                                                               \
    __device__ inline void potrf<CT, N, 0, ARCH, false>(CT* A, char* smem)       \
    {                                                                                         \
        _nvidia_chol_impl_##N##_##CT##_bd0_sm##ARCH::template run<false>(A, smem);                   \
    }                                                                                         \
    template <>                                                                               \
    constexpr std::size_t potrf_scratch_bytes<CT, N, 0, ARCH>()                         \
    {                                                                                         \
        return _nvidia_chol_impl_##N##_##CT##_bd0_sm##ARCH::smem_bytes;                              \
    }                                                                                         \
    template <>                                                                               \
    constexpr uint32_t potrf_threads<CT, N, 0, ARCH>()                              \
    {                                                                                         \
...
#define _GLASS_CHOL_BD(N, TC, CT, ARCH)                                                           \
    ...
    template <>                                                                               \
    __device__ inline void potrf<CT, N, TC, ARCH, true>(CT* A, char* smem)       \
    {                                                                                         \
        _nvidia_chol_impl_##N##_##CT##_bd##TC##_sm##ARCH::template run<true>(A, smem);               \
    }                                                                                         \
    template <>                                                                               \
    __device__ inline void potrf<CT, N, TC, ARCH, false>(CT* A, char* smem)      \
    {                                                                                         \
        _nvidia_chol_impl_##N##_##CT##_bd##TC##_sm##ARCH::template run<false>(A, smem);              \
    }                                                                                         \
    template <>                                                                               \
    constexpr std::size_t potrf_scratch_bytes<CT, N, TC, ARCH>()                        \
    {                                                                                         \
        return _nvidia_chol_impl_##N##_##CT##_bd##TC##_sm##ARCH::smem_bytes;                         \
    }                                                                                         \
    template <>                                                                               \
    constexpr uint32_t potrf_threads<CT, N, TC, ARCH>()                             \
    {                                                                                         \
...
//   potrs         — SPD solve given the L factor from potrf
...
// indirection + public DEFINE convenience-macro pattern as potrf/trsm.
...
// Solves L·L^T·X = B given the L factor from potrf.  B is overwritten
// with X.  Smem layout: [Ls: N*N] [Bs: N*NRHS] [solver_smem].

/**
 * @brief SPD solve given the Cholesky factor (cuSOLVERDx potrs).
 *
 * Solves `L*Lᵀ*X = B` given the lower factor L from potrf; B is
