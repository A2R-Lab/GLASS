...
// Promoted from RBD-SOLVER's floating-base integrator emitters, where this chain is
...
//     float64 oracle — the validated RBD-SOLVER design decision, kept).
//
...
namespace lie_detail {
    ...
    // from the pin.dIntegrate-validated RBD-SOLVER emitter). 3x3 column-major.
    template <typename T>
    __device__ __forceinline__ void se3_Q_block_core(const T *rho, const T *phi, T *Q) {
        ...
        if (t < static_cast<T>(1e-4)) {
            ...
            // verified against pin.dIntegrate(ARG1) in the RBD-SOLVER arc.
    ...
    //      mpmath-complex-step-validated RBD-SOLVER emitters) ------------------------
 ...
 * `pin.dIntegrate` ARG1 in the RBD-SOLVER arc this is promoted from). Series form
 * below θ = 1e-4.
 *
