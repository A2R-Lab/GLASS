...
// formulas are promoted verbatim from RBD-SOLVER's Pinocchio-validated codegen
...
//     Featherstone/RBD-SOLVER/MuJoCo ordering — note Pinocchio's USER-facing order
...
namespace spatial_detail {
    ...
    // the RBD-SOLVER `crm` emitter's (already column-major) index table, verbatim.
    template <typename T>
    ...
    // Row r of motion_cross(v)·x (the fused apply; RBD-SOLVER `crm_mul` rows, verbatim).
    template <typename T>
    ...
    // Entry (r, c) of force_cross(v) = −motion_cross(v)ᵀ (RBD-SOLVER `fx` table, verbatim).
    template <typename T>
    __device__ __forceinline__ T force_cross_entry(uint32_t r, uint32_t c, const T *v) {
        return -motion_cross_entry(c, r, v);
    }

    // Row r of force_cross(v)·f (RBD-SOLVER `fx_times_v` rows, verbatim).
    template <typename T>
    ...
    // Entry (r, c) of force_cross_dual(f) (RBD-SOLVER `icrf` table incl. its global
