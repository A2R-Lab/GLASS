 ...
 * step, shared by all K matrices). Used by TRAJOPT-SOLVER's Schur kernel
 * (Q_k, Q_kp1, R_k → K=3).
 *
 ...
 * the single-matrix `inv`. Used by TRAJOPT-SOLVER's Schur kernel (Q_k, Q_kp1, R_k).
...
namespace warp {
     ...
     * inversions from TRAJOPT-SOLVER/MPC-SOLVER into one block, one warp each). Per pivot:
