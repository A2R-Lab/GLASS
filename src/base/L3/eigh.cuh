 ...
 * for the batched-solver hot path (TRAJOPT-SOLVER's stage-local PSD projection): a FIXED
 ...
 *
 * Reference/parity oracle: `jacobi_study.py` (TRAJOPT-SOLVER so_sqp_prototype) —
 ...
 *
 * From the de-risk study on real TRAJOPT-SOLVER stage Hessians (n = 12..21): p95
 ...
 * wants (TRAJOPT-SOLVER computes `eps = 1e-6 * (1 + max(diag))` per block). The result
