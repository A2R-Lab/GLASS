...
(glass-dispatch.cuh, regenerated by `bench/tune.py --legs body`). Since the
2026-07-30 Phase-2 sweep, measured cells route to a warp-/thread-body executed
inside the block (wrappers in `src/base/dispatch.cuh`; rule: never worse than
block by >margin at ANY measured (batch, width) point, >margin faster at ≥1
width at the throughput batch, BOUNDED at the largest measured N; unmeasured
arches stay block everywhere). Ops with no moved cell are still the SAME
entities as the explicit block tier (re-exported via `using namespace block;`;
identity pinned in test_defaults.cu) — old spellings all still compile.
...
`src/base/dispatch.cuh`, or the bare face silently loses it. A moved cell
matches block to tolerance, NOT bit-exactly — emit `glass::block::` from
codegen and anywhere determinism is load-bearing.
