...
> **Anonymized for double-blind review.** The `pytest-gpu-proof` test
> attestation plugin is provided as an anonymized companion repository:
> <https://anonymous.4open.science/r/pytest-gpu-proof-C62D>. Download it and `pip install <extracted-dir>`
> before running the GPU test suite. Signed test receipts in `test/` are
> redacted here (signer identity and signature removed); signatures
> verify in the de-anonymized release.
...
is designed as a foundational linear-algebra layer for GPU robotics solvers.

📖 **Full documentation: <[ANONYMIZED-DOCS-URL]>** (source under [`docs/source/`](docs/source/)).
...
FetchContent_Declare(glass GIT_REPOSITORY [ANONYMIZED-REPO-URL])
...
[security model](https://anonymous.4open.science/r/pytest-gpu-proof-C62D)).
Details in
...
[hosted docs]([ANONYMIZED-DOCS-URL]) (sources in [`docs/source/`](docs/source/)):
