...
template<typename T, bool FULL_A>
__global__
void global_trsm_triangular_InPlace(uint32_t n,
                                    T *s_A,
                                    T *s_B) {
    glass::trsm_triangular<T, FULL_A>(n, s_A, s_B);
}
...
void global_trmm_left(uint32_t n,
                      ...
                      T *B,
                      T *C) {
    glass::trmm_left<T, TRANSPOSE_A>(n, k, alpha, A, B, C);
}

template<typename T, bool TRANSPOSE_A = false>
__global__
void global_trmm_right(uint32_t n,
                       uint32_t k,
                       T alpha,
                       T *A,
                       T *B,
                       T *C) {
    glass::trmm_right<T, TRANSPOSE_A>(n, k, alpha, A, B, C);
}
