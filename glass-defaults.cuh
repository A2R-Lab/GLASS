...
// ── sm_120 measured ladder (auto-regenerated by bench/tune.py) ──
// Source sweep: mega_sweep_20260626_2113.txt   tie margin: ±5% (nvidia must clear it)
...
constexpr backend ideal_sm120(op o, uint32_t N, bool f64) {
    switch (o) {
        ...
        case op::gemv:
            ...
            else      return N <= 48u ? backend::warp : backend::block;
        case op::gemm:
            if (!f64) return N <= 8u ? backend::warp : N <= 12u ? backend::block : N <= 64u ? backend::nvidia : backend::block;
            else      return N <= 4u ? backend::warp : backend::block;
        case op::chol:
