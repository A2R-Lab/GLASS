...
__global__ void k_syev(uint32_t n, const float* A, float* W, float* V) {
    ...
    glass::block::syev<float>(n, A, W, V, s);
}
...
__global__ void k_eig_clamp(uint32_t n, float* A, float eps) {
    extern __shared__ float s[];
    glass::block::eig_clamp<float>(n, A, eps, s);
}

#define SYEV_SIZES(F) F(1) F(2) F(3) F(4) F(7) F(12) F(16) F(32)

// ─── eigh / psd_project (fixed-sweep Jacobi; dtype-templated) ────────────────
// Consumer sizes 12/14/18/21 (TRAJOPT-SOLVER stage blocks) + small even/odd + an odd
...
__global__ void k_eigh_ct(const T* A, T* W, T* V) {
    ...
    glass::block::eigh<T, N>(A, W, V, reinterpret_cast<T*>(smraw));
}

template <typename T, uint32_t N>
__global__ void k_psd_project_ct(T* A, T eps) {
    extern __shared__ __align__(16) unsigned char smraw[];
    glass::block::psd_project<T, N>(A, eps, reinterpret_cast<T*>(smraw));
}
...
static int run_eigh(int n, int threads, const char* abin) {
    ...
    #define DN(N_) if (!ok && n == N_) { \
        k_eigh_ct<T, N_><<<1, threads, (int)glass::block::eigh_scratch_bytes<T, N_>()>>>(dA, dW, dV); ok = true; }
    EIGH_SIZES(DN)
    #undef DN
...
static int run_psd_project(int n, int threads, T eps, const char* abin) {
    ...
    bool ok = false;
    #define DN(N_) if (!ok && n == N_) { \
        k_psd_project_ct<T, N_><<<1, threads, (int)glass::block::psd_project_scratch_bytes<T, N_>()>>>(dA, eps); ok = true; }
    EIGH_SIZES(DN)
    #undef DN
...
int main(int argc, char** argv) {
    ...
    if (strcmp(op, "syev") == 0 || strcmp(op, "syev_ct") == 0) {
        ...
        int sm = (int)glass::block::syev_scratch_bytes<float>((uint32_t)n);
        if (strcmp(op, "syev") == 0) {
        ...
        int sm = (int)glass::block::eig_clamp_scratch_bytes<float>((uint32_t)n);
