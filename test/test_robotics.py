...
# Jlog3, the same references RBD-SOLVER and TRAJOPT-SOLVER validate against downstream.
...
# ─── IK-SOLVER wave: register argreductions, LDA rot ops, WORLD frame, gn_step ────
...
def test_quat_error_world_ik_solver_formula(bins):
    """The consumer adoption contract: a residual written log(q_des ⊗ q⁻¹)
    (IK-SOLVER's) equals quat_error<WORLD> with the ARGUMENTS SWAPPED."""
    q, qd = _quats(P), _quats(P)
...
def test_gn_step(bins, dtype, reg_diag):
    ...
    IK-SOLVER shape (6x7 — rank-deficient JᵀJ made SPD by the damping), both
